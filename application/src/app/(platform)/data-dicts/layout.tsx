/**
 * Route guard for /data-dicts.
 *
 * Uses the same effective-features computation as the platform layout so that
 * a user sees the Data Dictionary link in the nav if and only if they can
 * access the page.
 */
...
export default async function DataDictsLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!features['governance']) return <AccessDenied feature="Data Dictionary" />
  return <>{children}</>
}
